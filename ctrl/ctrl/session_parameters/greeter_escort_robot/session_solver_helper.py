...
class SolverHelper(SolverHelperSuper):
	...
	def classify_event_behavior(self,label):
		if label == "":
			...
			return "env"
		elif "speech" in label or "actions" in label or "movement" in label or "near_rob" in label:
	...
	def get_behavior_description(self, beh, beh_val, agent):
		if beh_val is None or beh_val is "False" or beh_val == False or beh_val == True:
			...
			if agent == "r":
				...
				elif beh == "h1_near_rob":
					return "The human gets close to the robot."
				elif beh == "h1_not_near_rob":
					return "The human leaves the robot."
				else:
					return ""
