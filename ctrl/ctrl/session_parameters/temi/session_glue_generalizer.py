...
class GlueGeneralizer(GlueGeneralizerSuper):
	...
	def generalize(self,glue,event_beh_classifier, main=False):
		...
		Rule 1: For states with a SINGLE outgoing transition of a particular event,
		        ...
		        Do NOT generalize termination actions, though
		...
		'''
		Rule 2: For states with MULTIPLE outgoing transitions of a particular event,
		        generalize the environmental state that they share
		(NOTE: if the state involves movement, do not generalize the position)
		'''
